...
export const message = (validator: ValidationResult, key: string, validationMessages?: Array<any>): string => {
    switch (key) {
        case 'required':
            return validationMessages[key] || 'Please enter a value';
        case 'pattern':
            return validationMessages[key] || 'Value does not match required pattern';
        case 'minlength':
            return validationMessages[key] || 'Value must be N characters';
        case 'maxlength':
            return validationMessages[key] || 'Value must be a maximum of N characters';
        case 'email':
            return validationMessages[key] || 'Invalid email address';
    }
